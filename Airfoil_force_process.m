...
%Sean Patrick Devey
%started:       3_10_20
%last updated:  3_26_20
%% TO DO:
%{  
check that negative angles are treated correctly in coordinate transform step and tunnel corrections
%}
...
%% Initialization (user inputs folder/list of files to post-process)
clear; clc; %close all;
...
% Control -8 to 8 run ----------------------
% data_folder = 'C:\Users\seanp\Documents\Box Sync\Everything\Documents\Research\Fall 2018 - Summer 2019\2019 wind tunnel experiment\Data\post processing\control_mini45\11_8_19\-8 to 8 deg'; %control no trip
% output_folder = 'C:\Users\seanp\Documents\Box Sync\Everything\Documents\Research\Fall 2019 - Spring 2020\Data\post-processing\control - no trip';
% theta = 82.4; %aoa offset = ~0.7 deg
%% loop to do everything with each run
system(['cd ',data_folder,' & ren *.xls *.dat']); %change file extension on all .xls files in the folder to prep fo data import to matlab
...
for k = 1:length(data_files)    %read each file one at a time
    ...
    %% loop for including tunnel corrections, bias correction 
    count=0;
...
system(['cd ',data_folder,' & ren *.dat *.xls']); %change file extension on all .xls files in the folder back for easy reading
%% Subfunctions (Load2LD and dependencies)
...
[row, ~] = size(data);
%% check for forces drift, remove rows of zero speed
zero_index = 0;
for i=1:row
...
if x >= 2 %% make sure there are at least two zero aoa rows
    data_0_aoa_mean = mean(data_0_aoa); % this is wrong when there is only one row of data in data_0_aoa
...
%this data set does not encompass the minimum loading condition (zero aoa, cdo). This means that the "correction" is not accurate at low loads, in particular low aoa drag data
